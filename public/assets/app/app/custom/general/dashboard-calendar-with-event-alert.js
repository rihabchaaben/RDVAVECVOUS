        ...
        $('#kt_calendar').fullCalendar({
            ...
            select: function (start, end) {
                ...
                if (title) {
                    ...
                    $('#kt_calendar').fullCalendar('renderEvent', eventData, true);
                }
                $('#kt_calendar').fullCalendar('unselect');
                var startformDate = moment(start).format('YYYY-MM-DD HH:mm:ss');
                var endformDate = moment(end).format('YYYY-MM-DD HH:mm:ss');
                $(".calendar-event").append('<div class="event"><input type="hidden" value="' + startformDate + '" name="events[start][]"><input type="hidden" value="' + endformDate + '" name="events[end][]"></div>')
            },

            eventAfterRender: function (event, element, view) {
                // $('.event').remove();
                // birthday = new Date('<somedate>');
                // var year = new Date(event.start).getFullYear();
                // var month = new Date(event.start).getMonth();
                // var day = new Date(event.start).getDate();
                // cosnole.log("Event ID: " + event.id + " Start Date: " +  $.fullCalendar.formatDate(event.start, 'MM-dd-yyyy') + " End Date: " + event.end);
                // var full = $.fullCalendar.formatDate(event.start, 'MM-dd-yyyy');
                // var startformDate = moment(event.start).format('YYYY-MM-DD HH:mm:ss');
                // var endformDate = moment(event.end).format('YYYY-MM-DD HH:mm:ss');
                // $(".calendar-event").append('<div class="event"><input type="hidden" value="' + startformDate + "'|'" + endformDate + '" name="events[]"></div>')


            },


            defaultDate: moment('2019-10-15'),
            events: [
                {
